...
class AE_Model(nn.Module):
    ...
    def initialize(self, opt):
        ...
        # use for test
        self.feature_list_male = np.fromfile(os.path.join(opt.param, opt.name) + '/man_' + opt.partial + '_feature.bin',
                                             dtype=np.float32)
        self.feature_list_male.shape = 6247, 512

        # girl
        self.feature_list_female = np.fromfile(
            os.path.join(opt.param, opt.name) + '/female_' + opt.partial + '_feature.bin', dtype=np.float32)
